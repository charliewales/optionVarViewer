...
class UI(object):
    def __init__(self):
        window = pm.window(title='optionVarEditor', widthHeight=(502, 552), s=False)
        parent = pm.columnLayout()
        rowClm = pm.rowColumnLayout(nc=2, cw=[(1, 250), (2, 250)])

        # List Column
        li = pm.columnLayout()
        self.search = pm.textField(width=248, cc=self.refresh, ec=self.refresh)
        self.scrollList = pm.textScrollList(height=500, width=250, selectCommand=self._select, parent=li, ams=True)

        # Attr Column
        pm.columnLayout(parent=rowClm)
        ...
        # Buttons
        pm.rowColumnLayout(nc=3, cw=[(1, 166), (2, 166), (3, 166)], parent=parent)
...
class dialog(object):
    def __init__(self, mainUI):
        self.mainUI = mainUI
        self.window = pm.window(title='Create')
        pm.columnLayout()
